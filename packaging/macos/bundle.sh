#!/bin/bash
# Build Fastsonic.app from a GUI binary, on a macOS machine.
#
...
cp "$binary" "$app/Contents/MacOS/fastsonic"
chmod 755 "$app/Contents/MacOS/fastsonic"
...
iconset="$(mktemp -d)/fastsonic.iconset"
mkdir -p "$iconset"
...
iconutil -c icns "$iconset" -o "$app/Contents/Resources/fastsonic.icns"
