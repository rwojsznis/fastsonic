...
mod tests {
    ...
    fn artwork_is_a_local_file() {
        let url = file_url(Path::new("/tmp/fastsonic/art/0badc0de"));
    ...
    fn art_arriving_is_a_change_worth_sending() {
        let bare = crate::media::MediaTrack {
            uri: "sonic:track:1".to_owned(),
            art_url: Some("sonic:art:640:album-1".to_owned()),
            ..Default::default()
        };
        let with_art = crate::media::MediaTrack {
            art_file: Some(std::path::PathBuf::from("/tmp/fastsonic/art/0badc0de")),
            ..bare.clone()
        };
